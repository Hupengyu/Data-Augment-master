...
IM_PATH = r"D:\2021-8-16\8-10翻越栏杆\image"

# path of save xml file
sa_path = r'D:\2021-8-16\8-10翻越栏杆'  # keep it blank
if not os.path.exists(sa_path):
    os.makedirs(sa_path)
# txt_folder is txt file root that using darknet rectbox
tx_folder = r'D:\2021-8-16\8-10翻越栏杆\xml'
...
for tc, dirs, files in os.walk(IM_PATH):
    # for file in files:
    # IMG_PATH = os.path.join(tc,file)
    IMG_PATH = tc
    txt_folder = tx_folder
    fw = os.listdir(IMG_PATH)
    if 'jpg' in fw[0]:
        print(tx_folder + '\\' + IMG_PATH[29:])
        pass
    else:
        continue
    save_path = r'D:\2021-8-16\8-10翻越栏杆'
    ...
    for line in fw:
        ...
        xml_paths = img_txt_root + '.xml'
        if os.path.exists(xml_paths):
            continue
        # print(txt_path)
        try:
        ...
        for ii in range(len(txt_file)):
            ...
            label = str(int(txt_file[ii][0]) + 1)  # 使用txt自带的label值
